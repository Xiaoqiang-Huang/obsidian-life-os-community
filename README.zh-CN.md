...
![版本](https://img.shields.io/badge/版本-0.3.17-0f172a?style=flat-square)
...
当前版本：**0.3.17** · Obsidian **1.5.0+** · `isDesktopOnly: false`

> **0.3.17 Agent 工具与工作流更新：** 桌面 AI 助手和微信 Bot 共用的 Life OS Agent 现已提供 38 个受权限控制的工具、持久任务与事件记忆、安全可复用工具配方、更完整的 Skill 路由、上下文压缩、附件生命周期和受控 Vault 写回。本次还新增提示词工作室、任务批量操作、知识库目录导入、可配置侧栏与备考模块，并修复多项任务、弹窗和渲染问题。

> **关于配图：** 下方导览图依据 0.3.17 当前的信息架构和已经实现的工作流生成，全部使用虚构数据和中性 Obsidian 主题。实际界面会随你的 Obsidian 主题、窗口尺寸和已启用功能变化。
...
图片会从受信任的微信 CDN 下载、解密并先保存为 Vault 本地附件，再发送给单独配置的视觉模型。由于微信需要把图片和说明分开发送，单独图片会自动与下一条文字配对；最近一批图片引用保留 3 天，可在后续消息甚至插件重启后被“用小P解前面那道题”重新调用，引用到期不会删除本地附件。会话状态只保存本地附件引用，带签名下载地址和图片字节不会进入 Markdown、持久收件箱或日志；可用 `/lifeos image-status` 查看、用 `/lifeos cancel-image` 清除。用户可直接说“小 P 是怎么算这道题的？”“我想用陈怀安”或“用花生十三解题”，Bot 会从当前已安装 Skill 中识别并为该微信会话记住选择，不继承电脑端默认 Skill；只有依赖上下文的追问才会沿用已选 Skill，切换到无关话题会回到中性方法，最终隔离检查会阻止未选老师串入答案。`/skill` 与 `@名称` 仍作为精确控制的兼容方式。所有回复都会把 LaTeX 改写为 `(a+b)/c`、`a*b` 等微信可读的普通算式。

微信 Bot 同时是受权限控制的 Life OS 远程工作台，而不是一套与产品割裂的聊天功能：直接用自然语言即可记日记、查看今日日记、管理待办、生成证据化复盘或总结、知识入库和设置提醒。“去官网查一下”这类追问会先结合上一轮问题改写成完整检索词，再执行带来源的网页检索；“把这个链接存一下”也会记住最近 URL，并在下一轮接续“存到 AI 研究”等目标。标记为无法访问、待读取正文或正文缺失的来源不能支撑事实结论；回复发送前还会校验引用是否包含结论、是否遵守当前 Skill，以及是否违反用户刚提出的偏好。`/lifeos ...` 命令仅作为兼容和诊断入口。提醒按 Bot 账号、发送者和会话隔离，并使用稳定发送 ID 与退避重试避免重复。
...
登录令牌只保存在当前 Vault 插件目录的多账号 `weixin-state.json`，不会进入 Markdown 或普通设置；移除账号只删除对应令牌，“退出全部账号”才会清空其余令牌。日终投递状态改为保存在 Obsidian 适配器可可靠读写的可见文件 `<Life OS 根目录>/Chat/Weixin/Daily/daily-digest-state.json`，旧隐藏文件会在首次读取时迁移。已授权入站消息会先脱敏写入 `Chat/Weixin/Inbox/` 持久队列；待处理、处理中或已生成但未送达的消息会在重连或重启后恢复，回复送达单独标记，避免重启吞消息。每个账号、发送者和会话的追问上下文、待完成收藏、Skill 与本地图片引用保存在 `Chat/Weixin/State/` 并相互隔离。实时回复、定时提醒和准时零点总结都要求桌面 Obsidian 保持运行；离线工作会在重连或启动后恢复。扫码创建的是独立 Bot 身份，不会读取个人微信聊天历史；私聊最稳定，普通群聊是否可用取决于微信通道能力。
...
创建 `.obsidian/plugins/personal-life-system/`，从精确语义版本 Tag（例如 `0.3.17`）中复制以下三个文件：
